    ...
    def __init__(self):
        ...
        if platform.system() == "Windows":
            ...
            if t_bits == '64bit':
                ...
                self.m_lb_path = "mwcapture\\bin\\x64\\"
                self.m_lb_path = os.getcwd() + "\\" + self.m_lb_path
            elif t_bits == '32bit':
                # self.m_lb_path = "mwcapture\\bin\\x86\\mw_mp4.dll"
                self.m_lb_path = "mwcapture\\bin\\x86\\"
                self.m_lb_path = os.getcwd() + "\\" + self.m_lb_path
            else:
                raise AssertionError("ERROR:unsupported architecture - %s"%(t_bits))
            try:
                # self.m_lib_mp4 = cdll.LoadLibrary(self.m_lb_path)
                os.chdir(self.m_lb_path)
                self.m_lib_mp4 = cdll.LoadLibrary("mw_mp4.dll")
