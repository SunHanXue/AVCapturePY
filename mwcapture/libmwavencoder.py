...
class mw_av_encoder(object):
    def __init__(self):
        ...
        self.m_abs_path = os.getcwd()
        if platform.system()=='Windows':
            if t_bits == '64bit':
                # self.m_lib_path = "mwcapture\\bin\\x64\\mw_av_encoderd.dll"
                self.m_lib_path = "mwcapture\\bin\\x64\\"
                self.m_lib_path = os.getcwd() + "\\" + self.m_lib_path
            elif t_bits == '32bit':
                # self.m_lib_path = "mwcapture\\bin\\x86\\mw_av_encoderd.dll"
                self.m_lib_path = "mwcapture\\bin\\x86\\"
                self.m_lib_path = os.getcwd() + "\\" + self.m_lib_path
            else:
                raise AssertionError("ERROR:Current Source doesn't support %s" % (t_bits))
            try:
                os.chdir(self.m_lib_path)
                self.m_lib_av_encoder = cdll.LoadLibrary("mw_av_encoderd.dll")
                os.chdir(self.m_abs_path)
